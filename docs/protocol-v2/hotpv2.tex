...
\begin{enumerate}
  \item \textit{Proxy-side Hottentot Protocol Major Version}, \texttt{FIXED LENGTH 1 byte}
  \item \textit{Proxy-side Hottentot Protocol Minor Version}, \texttt{FIXED LENGTH 1 byte}
  \item \textit{Flag byte}, \texttt{FIXED LENGTH 1 byte}
  \begin{itemize}
    \item \texttt{KeepAlive} flag, bit 0
    \item Bits from 1 to 7 are reserved for future usage.
  \end{itemize}
  \item \textit{Reserved}, \texttt{FIXED LENGTH 2 bytes}
  \item \texttt{Array} \textit{of supported} \texttt{SerializationAlgorithms}, \texttt{VARIABLE LENGTH}
  \item \texttt{Array} \textit{of proxy-side} \texttt{Certificates}, \texttt{VARIABLE LENGTH}
\end{enumerate}
}}
\end{flushleft}
...
\fbox{\parbox{\textwidth}{
\begin{enumerate}
  \item \textit{Service-side Hottentot Protocol Major Version}, \texttt{FIXED LENGTH 1 byte}
  \item \textit{Service-side Hottentot Protocol Minor Version}, \texttt{FIXED LENGTH 1 byte}
  \item \textit{Flag byte}, \texttt{FIXED LENGTH 1 byte}
  \begin{itemize}
    \item \texttt{Rejected} flag, bit 0
    \item Bits from 1 to 7 are reserved for future usage.
  \end{itemize}
  \item \textit{Reason byte}, \texttt{FIXED LENGTH 1 byte}
  \begin{itemize}
    \item \texttt{Proxy-side protocol version is too old} flag, bit 0
    \item \texttt{Proxy-side protocol version is too new} flag, bit 1
    \item \texttt{Serialization algorithms mismatch} flag, bit 2
    \item Bits from 3 to 7 are reserved for future usage.
  \end{itemize}
  \item \textit{Reserved}, \texttt{FIXED LENGTH 2 bytes}
  \item \texttt{Array} \textit{of proxy-side} \texttt{Certificates}, \texttt{VARIABLE LENGTH}
\end{enumerate}
