...
\begin{enumerate}
  ...
  \end{itemize}
  \item (\texttt{START: 0x03, LENGTH: 2 bytes}): \textit{Reserved for future usage and SHOULD be} \texttt{0x0000}
  \item (\texttt{START: 0x05, LENGTH: VARIABLE}): \texttt{Array} \textit{of supported} \texttt{SerializationAlgorithms}
  \item (\texttt{START: VARIABLE, LENGTH: VARIABLE}): \texttt{Array} \textit{of proxy-side} \texttt{Certificates}
\end{enumerate}
}}
\end{flushleft}
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item \texttt{BIT[0]}: \texttt{Proxy-side protocol version is too old} \textit{flag}
    \item \texttt{BIT[1]}: \texttt{Proxy-side protocol version is too new} \textit{flag}
    \item \texttt{BIT[2]}: \texttt{Serialization algorithms mismatch} \textit{flag}
    \item \texttt{BIT[3]} \textit{to} \texttt{BIT[7]} \textit{are reserved for future usage and SHOULD be} \texttt{0x00}
  \end{itemize}
  \item (\texttt{START: 0x03, LENGTH: 2 bytes}): \textit{Reserved for future usage and SHOULD be} \texttt{0x0000}
  \item (\texttt{START: 0x05, LENGTH: VARIABLE}): \texttt{Array} \textit{of supported} \texttt{SerializationAlgorithms}
\end{enumerate}
