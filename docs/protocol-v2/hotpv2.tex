...
Response \textit{Payload} is determined according to \textit{Request Type} and \texttt{Status Code}. When status code is not \texttt{0x0000}, payload is just a single \texttt{Exception} structure. If status code is \texttt{0x0000}, we have results and its structure is determined according to \textit{Request Type}.
