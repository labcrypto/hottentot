 ...
 * Name: data_wrapper.cc
 * Description:
 *   This file contains implementation of DataWrapper class.
 ******************************************************************/
...
namespace hotgen {
  unsigned char *
  DataWrapper::Serialize(uint32_t *length_ptr) {
    ...
    uint32_t length1 = 0;
    unsigned char *data1 = digi_.Serialize(&length1);
    if (length1 <= 127) {
  ...
  void
  DataWrapper::Deserialize(unsigned char *data, uint32_t length) {
    ...
    digi_.Deserialize(data + c, elength);
    c += elength;
    if (c != length) {
