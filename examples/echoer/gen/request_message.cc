...
namespace echoer {
  ...
  RequestMessage::Serialize(uint32_t *length_ptr) {
    ...
    } else if (length0 < std::numeric_limits<uint32_t>::max()) {
      totalLength += 5 + length0;
    }
    ...
    if (length0 < 128) {
      ...
      data[c] = 0x84;
      data[c + 1] = length0 / (256 * 256 * 256);
      data[c + 2] = (length0 - data[c + 1] * (256 * 256 * 256)) / (256 * 256);
      data[c + 3] = (length0 - data[c + 1] * (256 * 256 * 256) - data[c + 2] * (256 * 256)) / 256;
      data[c + 4] = length0 % (256 * 256 * 256);
      c += 5;
    }
  ...
  RequestMessage::Deserialize(unsigned char *data, uint32_t length) {
    ...
    if ((data[c] & 0x80) == 0) {
      ...
      if (ll == 1) {
        ...
        elength = data[c] * 256 * 256 * 256 + data[c + 1] * 256 * 256 + data[c + 2] * 256 + data[c + 3];
        c += 4;
      }
    }
